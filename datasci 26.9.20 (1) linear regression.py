...
def clean(arr,a):
    ...
    for i in range(0,len(arr)):
          ...
          for x in range(0,len(arr)):
                ...
                x = np.array(xx[0])
                y = np.array(xx[1])
                #plt.xlabel(arr[x][0])#enter debt as 0 for job stats
                #plt.ylabel(arr[i][0])
                #print(a[x])
                #print(a[i])
                plt.xlabel(x1)
...
compare=[]
clean(arr,a)
###now do prediction from correlation
#i will now  compare this data despite it being incomparable for some of the values
